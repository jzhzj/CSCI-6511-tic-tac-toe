...
public class MiniMax {
    ...
    private static double CheckCol(int x, int y, DefaultBoard board) {
        ...
        while (i > -1 && boardinfo[x][i].getType() == board.getLastPiece().getType()) {
            i--;
        }
        while (j < board.getWidth() && boardinfo[x][j].getType() == board.getLastPiece().getType()) {
            j++;
        }
        int continuous = j - i - 1;
        while (i > -1 && boardinfo[x][i].getType() == Piece.PieceType.Blank) {
            i--;
        }
        while (j < board.getWidth() && boardinfo[x][j].getType() == Piece.PieceType.Blank) {
            j++;
        }
        int window = j - i - 1;
    ...
    private static double CheckRow(int x, int y, DefaultBoard board) {
        ...
        i = j = x;
        Piece[][] boardinfo = board.getBoard();
        while (i > -1 && boardinfo[i][y].getType() == board.getLastPiece().getType()) {
            i--;
        }
        while (j < board.getWidth() && boardinfo[j][y].getType() == board.getLastPiece().getType()) {
            j++;
        }
        int continuous = j - i - 1;
        while (i > -1 && boardinfo[i][y].getType() == Piece.PieceType.Blank) {
            i--;
        }
        while (j < board.getWidth() && boardinfo[j][y].getType() == Piece.PieceType.Blank) {
            j++;
        }
        int window = j - i - 1;
        if (window < board.getGoal()) {
    ...
    private static double CheckDiagonalFromTopLeft(int x, int y, DefaultBoard board) {
        ...
        iy = jy = y;
        Piece[][] boardinfo = board.getBoard();
        while (ix > -1 && iy < board.getWidth() && boardinfo[ix][iy].getType() == board.getLastPiece().getType()) {
            ix--;
            iy++;
        }
        while (jx < board.getWidth() && jy > -1 && boardinfo[jx][jy].getType() == board.getLastPiece().getType()) {
            jx++;
            jy--;
        }
        int continuous = jx - ix - 1;
        while (ix > -1 && iy < board.getWidth() && boardinfo[ix][iy].getType() == Piece.PieceType.Blank) {
            ix--;
            iy++;
        }
        while (jx < board.getWidth() && jy > -1 && boardinfo[jx][jy].getType() == Piece.PieceType.Blank) {
            jx++;
            jy--;
        }
        int window = jx - ix - 1;
    ...
    private static double CheckDiagonalFromTopRight(int x, int y, DefaultBoard board) {
        ...
        iy = jy = y;
        Piece[][] boardinfo = board.getBoard();
        while (ix > -1 && iy > -1 && boardinfo[ix][iy].getType() == board.getLastPiece().getType()) {
            ix--;
            iy--;
        }
        while (jx < board.getWidth() && jy < board.getWidth() && boardinfo[jx][jy].getType() == board.getLastPiece().getType()) {
            jx++;
            jy++;
        }
        int continuous = jx - ix - 1;
        while (ix > -1 && iy > -1 && boardinfo[ix][iy].getType() == Piece.PieceType.Blank) {
            ix--;
            iy--;
        }
        while (jx < board.getWidth() && jy < board.getWidth() && boardinfo[jx][jy].getType() == Piece.PieceType.Blank) {
            jx++;
            jy++;
        }
        int window = jx - ix - 1;
